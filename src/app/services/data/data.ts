...
//import { Observable } from 'rxjs/Observable';7
...
export class DataProvider {
  ...
  constructor(private Http: HttpClient) 
    ...
    this.DataSourceTypes=[];
		...
		this.ourserveraddress="192.168.2.26:8099"
  }
 ...
 reloadData(refresh,callback)
 {
		...
		this.http.get(url).subscribe(data => {  
			    this.Tags=data;			   
				  console.log("have data back for tags="+JSON.stringify(data));
					// this callback will be called asynchronously
					// when the response is available
					url='http://'+this.ourserveraddress+'/datasources'
					this.http.get(url).subscribe(data1 => {
							console.log("have data back for datasources="+JSON.stringify(data1));
							this.DataSources= data1;			          			                             
							url='http://'+this.ourserveraddress+'/viewers'
							this.http.get(url).subscribe(data2 => {
									console.log("have data back for viewers="+JSON.stringify(data2));
										this.Viewers= data2;
										url='http://'+this.ourserveraddress+'/images'
										this.http.get(url).subscribe(data3 => {
										  console.log("have data back for images="+JSON.stringify(data3));
													this.Images= data3;													
													if(refresh == true)
													{
