...
import { NavparamService } from '../services/navparam/NavparamService';
...
export class StartupPage  {
 chrome: any;
  ...
  constructor(private navCtrl: NavController,
     ...
     this.serveraddress='192.168.2.26:8099';
     this.port='';
     this.d=data;
  ...
  getServerAddress()
  {
  		...
  		var promise=new Promise(function(resolve,reject)
  		{
  			...
  			try 
  			{
						self.modalCtrl.create({component:'AddressModalPage',componentProps:{}})
				   .then(	(res) =>{
				 console.log("dialog created");
						res.onDidDismiss().then((updatedObject) =>	{
  ...
  loadData(server)
  {
  	var self=this
		...
		self.data.reloadData(true, (error) => 			{
		  ...
		  self.navCtrl.navigateRoot('/home');
			/*self.navCtrl.push('HomePage') .then( 	 () => 	 {
		 			  console.log("new page loaded");
			 		 // first we find the index of the current view controller:
						const index = self.modalCtrl.index;
  ...
  findServerAddress(ip,cb)
	{
       ...
       var callback=cb;
       var counter=0;
       var server_address="192.168.2.38:8099";
