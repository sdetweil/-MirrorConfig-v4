import { Component, OnInit } from '@angular/core';import { NavParams } from '@ionic/angular';
...
d: any;
  ...
  doubleClick(index,item)
  {
	...
	saveModal()
	{
		this.modalCtrl.dismiss({type:this.modaltype,data:this.thisobject,path:this.parentpath,files:this.files,action:'save'}/* some data, thisdatasource has been updated*/);
	}
      ...
      let self=this;
			...
			if((this.foldersOnly && this.files[index].filetype=='Folder') || this.foldersOnly==false)
			{
  			let i=this.files[index].selected;
				switch(i)
				{	
